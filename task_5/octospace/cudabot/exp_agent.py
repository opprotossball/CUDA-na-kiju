...
def target_direction(ship: Ship, target: Ship):
    for direction in range(4):
        ...
        vec_angles = [np.arccos(
            np.clip(np.dot(vec / np.linalg.norm(vec), target_vec / np.linalg.norm(target_vec)), -1.0,
                    1.0)) if np.linalg.norm(vec) != 0 else 0 for vec in vec_to_other_ships]
